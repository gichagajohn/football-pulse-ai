...
Hashtags are always appended locally via build_hashtags() for consistency —
Gemini is only used to write the caption text + engagement question.
"""
...
def generate_with_gemini(prompt: str) -> Optional[str]:
    ...
    NOTE: Gemini is asked NOT to include hashtags. Hashtags are appended
    separately via build_hashtags() so they are always present and consistent.
    """
    ...
    try:
                ...
                "contents": [{
                    "parts": [{
                        "text": (
                            ...
                            "Write a punchy, engaging caption for a football post, ending with a fan "
                            "engagement question. Keep it under 150 words. "
                            "Do NOT include any hashtags - hashtags are added separately. "
                            "No emojis that might not render correctly.\n\n"
                            + prompt
                        )
                    }]
                }],
        ...
        # Safely navigate the response — Gemini 2.5-flash sometimes returns
        # candidates with no content when finish_reason is not STOP
        candidates = data.get("candidates", [])
        if not candidates:
            logger.warning("Gemini returned no candidates — falling back to template.")
            return None

        candidate = candidates[0]
        finish_reason = candidate.get("finishReason", "")
        if finish_reason not in ("STOP", "MAX_TOKENS", ""):
            logger.warning("Gemini finish_reason=%s — falling back to template.", finish_reason)
            return None

        content = candidate.get("content", {})
        parts = content.get("parts", [])
        if not parts:
            logger.warning("Gemini response missing parts — falling back to template.")
            return None

        text = parts[0].get("text", "").strip()
        if not text:
            logger.warning("Gemini returned empty text — falling back to template.")
            return None

        # Safety net: strip any hashtags Gemini may have added anyway
...
def generate_fulltime_caption(
    ...
    if gemini_key:
        prompt = (
            ...
            f"Ask fans for their player ratings."
        )
        result = generate_with_gemini(prompt)
        if result:
            return f"{result}\n\n{hashtags}"
...
def generate_transfer_caption(
    ...
    if gemini_key:
        prompt = (
            ...
            f"Ask fans their opinion on the transfer."
        )
        result = generate_with_gemini(prompt)
        if result:
            return f"{result}\n\n{hashtags}"
...
def generate_fact_caption(fact_text: str) -> str:
    ...
    if gemini_key:
        prompt = (
            ...
            f"Fact: {fact_text}.\n"
            f"Ask fans if they knew this fact."
        )
        result = generate_with_gemini(prompt)
        if result:
            return f"{result}\n\n{hashtags}"
